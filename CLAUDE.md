...
This is a fork of upstream `codebase-memory-mcp`. The goal of this fork is to produce a **separate binary** that has all MCP protocol handling, the coordination daemon, and networking (Unix-domain sockets, the graph-viz HTTP server) removed, so the resulting tool satisfies "no MCP servers" AI policies at companies that ship this internally. Two constraints drive every change here:
...
2. **No networking, no MCP, no daemon.** The shipped binary must not open sockets, start an HTTP server, or speak MCP JSON-RPC. That means `src/mcp/`, `src/daemon/` (Unix-domain socket IPC, session coordination), and `src/ui/` (the graph-viz httpd) are the removal targets — not `src/cli/` (local one-shot commands), which upstream already treats as network-free (see below).

When in doubt about whether a change belongs in this fork vs. upstream: behavior changes that only strip MCP/daemon/UI/networking belong here; general indexing/parsing/language improvements should go upstream and be pulled in.

## Commands

```bash
scripts/build.sh              # standard release build -> build/c/codebase-memory-mcp
scripts/build.sh --with-ui    # only relevant upstream; this fork should not need --with-ui
...
src/
  ...
  ui/           graph-visualization HTTP server (first-party httpd, served at localhost:9749)  <- REMOVAL TARGET
...
graph-ui/       React/Three.js frontend for the graph UI — only relevant if src/ui is kept
```
...
- **UI** (`src/ui/`): serves the bundled graph-viz frontend over HTTP on `localhost:9749`; owned by the daemon so concurrent sessions don't start duplicate servers. Out of scope for a no-networking binary.
